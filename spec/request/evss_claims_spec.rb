...
RSpec.describe 'EVSS Claims management', type: :request do
  ...
  context 'for a user without evss attrs', skip_mvi: true do
    let(:va_profile) { build(:va_profile, edipi: nil) }
    before { allow_any_instance_of(MVI::Service).to receive(:find_candidate).and_return(va_profile) }
